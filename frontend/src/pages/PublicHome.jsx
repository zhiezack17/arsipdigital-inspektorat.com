...
import {
    ArrowRight, CalendarDays, ChevronRight, Clock3, FileText,
    Images, LogIn, Megaphone, Newspaper, ShieldCheck, Users,
} from 'lucide-react';
...
import { LOGO_KAB_ROHIL } from '@/lib/branding';
...
export default function PublicHomePage() {
    ...
    return (
        <PublicLayout>
                ...
                <div className="relative mx-auto grid min-h-[520px] max-w-7xl items-center gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1.25fr_.75fr] lg:px-8 lg:py-20">
                    <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: .65 }}>
                        <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold tracking-wide backdrop-blur">
                            <ShieldCheck className="h-4 w-4 text-[#f5c451]" /> PORTAL RESMI INSPEKTORAT DAERAH
                        </div>
                        ...
                        <h1 className="max-w-4xl font-heading text-4xl font-bold leading-[1.08] tracking-tight sm:text-5xl lg:text-6xl">
                            Informasi Pengawasan yang Transparan, Modern, dan Terpercaya
                        </h1>
                        <p className="mt-6 max-w-2xl text-base leading-7 text-white/80 sm:text-lg">
                            Pusat berita, pengumuman, agenda, dan informasi publik Inspektorat Daerah Kabupaten Rokan Hilir.
                        </p>
                        <div className="mt-8 flex flex-wrap gap-3">
                            <Button asChild size="lg" className="bg-[#f5c451] font-semibold text-[#064027] hover:bg-[#ffd875]">
                                <Link to={hero ? `/berita/${hero.slug}` : '/berita'}>
                                    <Newspaper className="mr-2 h-5 w-5" /> Baca Informasi Terbaru
                                </Link>
                            </Button>
            ...
            <section className="bg-[#073e27] py-14 text-white">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="grid gap-8 text-center sm:grid-cols-2 lg:grid-cols-4">{[['1','Portal Informasi'],['100%','Komitmen Integritas'],['24/7','Akses Informasi'],['6','Unit Layanan Arsip']].map(([value,label]) => <div key={label}><p className="font-heading text-4xl font-bold text-[#f5c451]">{value}</p><p className="mt-2 text-sm text-white/70">{label}</p></div>)}</div></div>
            </section>
